...
export default function CelebrityStyleSection({ type }) {
  ...
  const visible = celibrity_style?.length > 4 ? 4.3 : 4;
      ...
      {!mobileView ? (
        <CustomSection style={{ paddingTop: '3rem', paddingBottom: '3rem' }}>
          ...
          <CarouselProvider
            ...
            totalSlides={celibrity_style.length}
        ...
        <CustomSection
          style={{ backgroundImage: `url("${dottedBg}")`, marginTop: '14px' }}
        >
          <div className={styles.title}>
            <h4>Celebrity Style</h4>
            <CustomDivider
              style={{ marginLeft: '5px', height: '1px', background: '#ffff' }}
            />
          </div>
          <div>
