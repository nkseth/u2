...
function Rating({ item, id, set_is_reviewed }) {
    ...
    const on_submit = async () => {
        ...
        try {
            const { data } = await common_axios.post('/product_review', {
                form
            }, {
                headers: {
    ...
    return (
        <>
            <div className={styles.productRatingDiv}>
                ...
                <ReactStars value={point} onChange={(e) => setPoint(e)} size={40} edit={true} />
            </div>
            <div className={styles.productReviewDiv}>
                <span>Review this product</span>
                <textarea value={review} onChange={(e) => setReview(e.target.value)} row={5} placeholder='Write your review' />
            </div>
            <div className={styles.addImgDiv}>
                <div>
                    <input ref={image_ref} accept='image/*' type='file' id='addImg' onChange={_onChange} />
            ...
            <Button onClick={on_submit} variant='text' color='default' className={styles.button}>
                Submit
            </Button>
        </>
    )
